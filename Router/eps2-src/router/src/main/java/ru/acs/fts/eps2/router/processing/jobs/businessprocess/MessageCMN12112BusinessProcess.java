...
import ru.acs.fts.schemas.album.uploadedgoodsdepartrequest.UploadGoodsDepartRequestType;

/**
 * Бизнес процесс обработки сообщения CMN.12112
 * ( Запрос на убытие ВС(контейнер) )
 */
public class MessageCMN12112BusinessProcess extends BusinessProcess
{
		...
		throws Exception
	{
		...
		UploadGoodsDepartRequestType departRequest = null;
		...
		for ( String cDocId : recvEnv.getDocument( ).getContainerDocuments( ).keySet( ) )
		{
			...
			else if ( cdoc.isDocumentOfClass( UploadGoodsDepartRequestType.class ) )
				departRequest = cdoc.getDocumentAsClass( UploadGoodsDepartRequestType.class );
			else
				continue;
