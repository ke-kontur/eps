...
public class MessageADM00004_CMN13001BusinessProcess extends BusinessProcess
{
		...
		throws Exception
	{
		...
		if ( ! jobBatchContext.contains( ContextConstants.ENVELOPES ) )
			createAndPutEnvelopes( jobBatchContext, jobContext );
		if ( ! jobBatchContext.contains( ContextConstants.ENVELOPES_UNSIGNED ) )
			createAndPutEnvelopesUnsigned( jobBatchContext, jobContext );
	}
