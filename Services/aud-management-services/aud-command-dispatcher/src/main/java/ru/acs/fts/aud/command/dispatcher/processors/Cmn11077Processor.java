...
import ru.acs.fts.aud.service.model.defines.MessageTypes;

public class Cmn11077Processor extends BaseProcessor
{
	private static final Logger log = LoggerFactory.getLogger( Cmn11077Processor.class );
		...
		throws Exception
	{
		...
		List< AudCommandCreationContext > saveMsg11034Ctxs = new ArrayList< AudCommandCreationContext >( );		
		try
		{
			...
			List< EdeclEnvelope > envelopes = _edeclService.getEnvelopesByMessageTypeAndProcessId( MessageTypes.MSG_11034, procInfo.getProcessId( ) );
			for ( EdeclEnvelope env : envelopes )
			{
				AudCommandCreationContext ctx = DataAccess.createCommandForDocument( 
					env.getDocument( ).getDocumentId( ), procInfo, false, intCommand.getMsgCode( ) 
				);
				
				EdeclAudCommand cmd = ctx.getAudCommand( );
				cmd.setIntCmd( intCommand );
				cmd.setState( AudCommandStates.WAITABLE );
				cmd.setEadFlag( 0 );				
				persistAudCommandCreationContext( ctx );
				
				saveMsg11034Ctxs.add( ctx );
			}
		...
		catch ( Exception exc )
		{
			...
			try
			{
				...
				for ( AudCommandCreationContext ctx : saveMsg11034Ctxs )
				{
