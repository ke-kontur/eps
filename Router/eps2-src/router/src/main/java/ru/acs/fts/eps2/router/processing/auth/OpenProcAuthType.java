...
public class OpenProcAuthType extends AbstractDeclToCustomsAuthType
{
		...
		throws ResultTypeException, DatabaseException 
	{
		...
		checkReceiverCustomsValidness( env );
	...
	protected void receiverCustomsChecking( EDJobBatchContext jobBatchContext )
		throws ResultTypeException, DatabaseException
	{
		String exchType = jobBatchContext.getConfigurator( ).getAsktt2ExchType( );
		String recvExchType = jobBatchContext.getReceivedEnvelope( ).getReceiverCustoms( ).getExchType( );
		
		if ( ! recvExchType.equalsIgnoreCase( exchType ) )
			super.receiverCustomsChecking( jobBatchContext );
	}
	
	protected void checkReceiverCustomsValidness( EDEnvelope env, EDJobBatchContext jobBatchContext )
		throws ResultTypeException, DatabaseException
	{
		CustomsType receiverCustoms = env.getReceiverCustoms( );
		
		String asktt2ExchType = jobBatchContext.getConfigurator( ).getAsktt2ExchType( );
		String recvExchType = receiverCustoms.getExchType( );
		
		if ( ! StringUtil.isNullOrEmpty( asktt2ExchType ) && 
			 ! asktt2ExchType.equalsIgnoreCase( recvExchType ) )
			checkReceiverCustomsValidness( env );
	}
