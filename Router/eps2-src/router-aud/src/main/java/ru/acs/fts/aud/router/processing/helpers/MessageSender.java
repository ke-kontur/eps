...
public class MessageSender
{
	...
	public static void send( AudJobBatchContext jobBatchContext, JobContext jobContext, AudEnvelope envelope ) 
		throws Exception
	{
		if ( null == envelope )
			ExceptionHelper.throwException( "Не указан конверт для отправки." );
	...
	private static void makeMqRoutingInfo( AudEnvelope receivedEnvelope, AudEnvelope envelope, JobContext jobContext ) 
	...
	{		
		try
		{
			...
			ExternalRoutingInfo mqRoutingInfo = getRoutingAddress( 
				/* receivedEnvelope */receivedEnvelope, 
				/* envelopeToSend */envelope, 
				/* sender */envelope.getSenderSystem( ), 
				/* receiver */envelope.getRecipientSystem( )
			);
	...
	private static ExternalRoutingInfo getRoutingAddress( 
			AudEnvelope recvEnv, AudEnvelope envelopeToSend,
			String senderSyste, String recipientSystem )
	{
