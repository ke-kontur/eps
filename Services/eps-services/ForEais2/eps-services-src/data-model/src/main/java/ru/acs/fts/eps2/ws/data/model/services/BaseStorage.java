...
public class BaseStorage< T >
{
			...
			final int offset, final int limit )
	{
		try
		{
			return ( List< T > )jpaTemplate.execute( new JpaCallback( )
			{
				public Object doInJpa( EntityManager em )
				{
					Query query = em
		...
		{
			String errorMsg = String.format( "Ошибка SELECT в БД: %s", exc.getMessage( ) );			
			log.error( errorMsg );
	...
	@SuppressWarnings( { "unchecked", "rawtypes" } )
	public List< ? > selectSpecialSlice(
			final String queryString,
			final Map< String, Object > args,
			final int offset, final int limit )
	{
		try
		{
			return ( List< ? > )jpaTemplate.execute( new JpaCallback( )
			{
				public Object doInJpa( EntityManager em )
				{
					Query query = em
